...
public:
  ...
  uint32_t convertToEpochtime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minutes, uint8_t seconds);
  ...
  uint8_t addGPSplus(uint8_t channel, float latitude, float longitude, float altitude, uint8_t satellites, uint16_t speed, long timestamp);
